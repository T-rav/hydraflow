...
created_at: 2026-07-26T12:12:36.633825+00:00
status: active
corroborations: 1
...
Example: `passed` and `has_fixable_only` stay keyed on `issues` only, so `adr_reviewer.py`'s council-skip check (line 183) never sees advisories. See also: this topic — "adr_reviewer.py logs pre-validation advisories via logger.warning".
