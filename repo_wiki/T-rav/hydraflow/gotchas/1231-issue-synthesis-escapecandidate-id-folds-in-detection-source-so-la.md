...
created_at: 2026-07-26T12:12:37.346686+00:00
status: active
corroborations: 1
...
In `src/escape/metrics.py`, `EscapeCandidate.id` embeds `detection_source` (e.g. `bug-issue:sha` vs `regression-pin:sha`), so `latest_by_id()` treats re-detection of the same commit under a stronger source as a brand-new row instead of a correction.

Example: fix keys identity on the escaping commit (`detection_ref`) via a new `latest_by_escape` in metrics, not on the source-qualified id.
