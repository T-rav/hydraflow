...
created_at: 2026-07-26T12:12:36.431966+00:00
status: active
corroborations: 1
...
Example: FAILURE (red CI) → `PRRedRepairLoop`; DIRTY (merge conflict) → `MergeStateWatcher` + arch-regen self-heal; green-but-unmerged → `DependabotMergeLoop` class-5 path. Before adding a new watcher, check which of these three already owns it — see `docs/wiki/patterns.md` entry "Open-PR failure modes each have an owning shepherd loop".
