...
created_at: 2026-07-26T12:12:36.462293+00:00
status: active
corroborations: 1
...
Example: this harness drives the real `adr_index.parse_adr_file` + `adr_drift.compute_drift` against actual ADR text, so it validates production parsing behavior, not a mock. See also: this topic — "ADR-drift right-sizing regression tests pin exact symbol owners".

**Why:** Duplicating harnesses per-ADR was flagged as a gotchas-audit risk; the shared parametrized table is the established pattern.
