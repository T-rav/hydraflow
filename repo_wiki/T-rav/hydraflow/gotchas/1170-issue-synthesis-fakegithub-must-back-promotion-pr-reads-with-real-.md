...
created_at: 2026-07-26T12:12:36.439405+00:00
status: active
corroborations: 1
...
Example: a standard `create_pr` (issue_number>0) must never be returned by `find_open_promotion_pr` — distinguish by explicit ID set, not base-branch/issue heuristics.

**Why:** Silent stub returns make MockWorld-tier promotion scenarios pass trivially without exercising the real read path.
