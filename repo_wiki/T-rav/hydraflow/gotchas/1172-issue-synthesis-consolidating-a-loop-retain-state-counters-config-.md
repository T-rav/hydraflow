...
created_at: 2026-07-26T12:12:36.444510+00:00
status: active
corroborations: 1
...
# Consolidating a loop: retain state counters/config caps, drop only cadence
...
Example: `PrRedRepairLoop` → `PRUnstickerLoop` intake keeps `PRPort.rerun_workflow_failed`, `state/_pr_red_repair.py` ConvergenceLedger counters, and `pr_red_repair_*` enable-flags/caps in `src/config.py`, but drops `pr_red_repair_interval`. See also: this topic — "Loop consolidation is a two-pass _do_work".
