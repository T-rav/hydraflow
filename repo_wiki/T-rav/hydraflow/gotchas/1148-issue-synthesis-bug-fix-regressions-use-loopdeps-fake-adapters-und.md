...
created_at: 2026-07-26T12:12:36.136968+00:00
status: active
corroborations: 1
...
Follow the established pattern: direct loop instantiation using `LoopDeps` wired to `Fake*` adapters (e.g. `FakeGithub`), placed under `tests/regressions/` — not a separate `tests/scenarios/` file unless the bug is loop-integration-shaped.

Example: matches prior precedent like `regression_issue_10225.py`. See also: testing — bug fixes land with a regression test in `tests/regressions/`.
