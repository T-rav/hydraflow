...
created_at: 2026-07-26T12:12:36.449610+00:00
status: active
corroborations: 1
...
Example: `PRUnstickerLoop._do_work` runs the existing HITL unstick pass, then calls `PrRedIntake.run_once()` as a second, independently gated pass (skippable via `PRUnstickerLoop`'s kill-switch or `pr_red_repair_loop_enabled`). See also: this topic — "Consolidating a loop: retain state counters/config caps".
