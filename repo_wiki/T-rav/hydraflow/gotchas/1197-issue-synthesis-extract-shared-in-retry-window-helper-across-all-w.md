...
created_at: 2026-07-26T12:12:36.753854+00:00
status: active
corroborations: 1
...
Example: `_is_safe_to_gc` (state phase) and `_collect_orphaned_branches` (orphan `agent/issue-N` branch phase) must share the helper. See also: this topic — "WorkspaceGCLoop must check every attempt counter".

**Why:** GC has multiple independent sweep phases; a guard fixed in one but not extracted to a shared helper silently reappears as a bug in the others.
