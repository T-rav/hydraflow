...
created_at: 2026-07-26T10:44:02.122497+00:00
status: active
corroborations: 1
...
`TermProposerLoop` auto-adds `docs/wiki/terms/*.md` drafts whose aliases can duplicate phrasing already present as prose in files like `gotchas.md` or `dark-factory.md`, tripping `test_paraphrase_lint_runs_against_live_wiki` and failing both Tests and Coverage jobs — which roll up into a red CI Gate on the next `rc/*` promotion PR.
