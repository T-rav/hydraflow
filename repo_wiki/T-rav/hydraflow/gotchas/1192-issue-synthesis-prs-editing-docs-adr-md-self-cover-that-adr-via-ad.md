...
created_at: 2026-07-26T12:12:36.652331+00:00
status: active
corroborations: 1
...
Example: when writing regression tests for ADR-drift fixes (e.g. issue #10443's fix to ADR-0055:107), the test only needs to check a *separate* file-only diff (touching `src/base_background_loop.py` alone) to reproduce the drift.
