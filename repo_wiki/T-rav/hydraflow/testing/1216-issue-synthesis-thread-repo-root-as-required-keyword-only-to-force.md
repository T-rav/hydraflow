...
created_at: 2026-07-26T12:17:08.227200+00:00
status: active
corroborations: 1
...
Example: `active_lint_tracked` itself can default `repo_root=None` for backward compat with 15 existing call sites, but the loop-level call must pass `repo_root=worktree` explicitly.

**Why:** If every layer defaults to `None`, the shipped-claim exemption silently never fires in production even though all unit tests pass against the default fallback.
