...
created_at: 2026-07-18T15:01:50.863652+00:00
status: active
corroborations: 1
...
In phase-runner integration tests, wire real `StateTracker`, `EventBus`, `VerificationJudge`, and `RetrospectiveCollector`. Mock only the `_execute()` subprocess boundary with configurable transcript strings. Validate via `StateTracker` APIs and `EventBus.get_history()`, not mock call assertions.
