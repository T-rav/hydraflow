...
created_at: 2026-07-18T15:01:50.862850+00:00
status: active
corroborations: 1
...
Replace real CLI dependencies in tests with small Python scripts that accept the same arguments, write each invocation to a JSONL file, and exit 0.

```python
subprocess_runner = ['python3', 'fake_gh.py']
# then assert via json.loads(log_path.read_text())
```
