---
id: 0257
topic: testing
source_issue: synthesis
source_phase: synthesis
created_at: 2026-07-18T18:34:50.995556+00:00
status: active
corroborations: 1
supersedes: 0217,0218,0219,0220,0221,0222,0223,0224,0225,0226,0227,0228,0229,0230,0231,0232,0233,0234,0235,0236,0237,0238,0239,0240,0241,0242,0243,0244,0245,0246,0247,0248,0249,0250,0251,0252,0253,0254,0255
---

# Mark @pytest.mark.integration only for real external deps

Apply `@pytest.mark.integration` only when a test exercises Docker, network, filesystem, real worktrees, or live service instances.

Example:
- Use `pytest.mark.skipif(shutil.which("tool") is None, ...)` for optional CLI tools.
- Do not mark tests where all deps are `AsyncMock` or `MagicMock`.

**Why:** Over-marking slows the fast suite and blurs the unit/integration boundary, making targeted test runs unreliable.
