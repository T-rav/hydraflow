...
created_at: 2026-07-18T18:34:51.022484+00:00
status: active
corroborations: 1
...
Every toggle requires both a field in `src/config.py` AND an entry in `_ENV_INT_OVERRIDES`.

Example: Test both the default value and the env-var override path. The `_ENV_INT_OVERRIDES` tuple default must equal the Field default. See also: architecture-state-persistence — `_ENV_INT_OVERRIDES` default sync.
