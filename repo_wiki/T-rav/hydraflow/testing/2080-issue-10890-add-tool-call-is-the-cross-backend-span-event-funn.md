...
source_issue: 10890
source_phase: plan
created_at: 2026-07-31T12:12:42.365154+00:00
status: active
corroborations: 1
---
...
- `bridge_event_to_span` (src/telemetry/subprocess_bridge.py:30) only matches top-level `type == "tool_use"` — no backend emits that shape, so the bridge stays inert if you emit at `_record_inner`.
- Emit `{"type": "tool_use", "tool": <name>, "name": <tool_use_id>}` so the bridge contract and its unit tests stay untouched.
