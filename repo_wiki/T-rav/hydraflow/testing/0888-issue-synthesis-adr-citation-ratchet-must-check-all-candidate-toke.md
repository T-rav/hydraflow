...
created_at: 2026-07-24T15:47:48.037951+00:00
status: active
corroborations: 1
...
Example: add a token-parity ratchet that extracts every lenient `src/*.py`-like candidate substring per ADR and asserts `_SOURCE_FILE_CITATION_RE.fullmatch` succeeds, excluding placeholders via `_is_glob` (e.g. `src/<module>.py:<Symbol>`).
