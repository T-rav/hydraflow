...
created_at: 2026-07-18T04:14:32.521060+00:00
status: active
corroborations: 1
...
# Feature toggles require config field and _ENV_INT_OVERRIDES entry

Every toggle requires both a field in `config.py` AND an entry in `_ENV_INT_OVERRIDES`. The tuple default must equal the Field default or the override silently stops applying. Test both the default value and the env-var override path.
