...
created_at: 2026-07-18T15:01:50.867120+00:00
status: active
corroborations: 1
...
# Sync test label/stage constants with production pipeline definitions

Keep test constants (`ALL_PIPELINE_LABELS`, `VALID_STAGES`, `VALID_TRANSITIONS`) synchronized with production definitions. Use dynamic length and set-equality checks:

```python
assert len(LABELS) == len(ALL_PIPELINE_LABELS)
assert set(VALID_TRANSITIONS.keys()) == VALID_STAGES
```
