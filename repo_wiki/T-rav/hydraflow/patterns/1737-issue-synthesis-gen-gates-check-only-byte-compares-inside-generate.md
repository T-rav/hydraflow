...
Example: A README can say "2 required checks" two lines below a generated table showing 3, and `gen-gates-check` stays green. Add a separate prose validator (`validate_prose_counts` in `scripts/gates/validate.py`) wired into the `violations` list in `scripts/gen_gates.py:main()` to close the gap.
