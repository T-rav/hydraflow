...
created_at: 2026-07-18T14:57:29.444954+00:00
status: active
corroborations: 1
...
# Memory eviction must atomically update both scores and items files
...
Example: `atomic_write(scores_path, ...)` then `atomic_write(items_path, ...)` under a single lock acquire. See also: patterns — Use `file_util.atomic_write()` for critical state file updates.
