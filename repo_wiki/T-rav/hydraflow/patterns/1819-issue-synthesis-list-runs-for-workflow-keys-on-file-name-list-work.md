...
created_at: 2026-08-14T12:50:04.250786+00:00
status: superseded
corroborations: 1
supersedes: 1723
superseded_by: 1917
...
Example: `ports.py` and `pr_manager.py` docstrings must each name which identifier their read keys on. Querying `list_runs_for_workflow` with a display name returns `[]` plus one WARNING naming the namespace mistake. See also: [patterns] — FakeGitHub tracks workflow display name and file name separately.
