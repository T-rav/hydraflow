...
In `src/prompt_efficiency.py`, the zero-usage path used a bare `window_calls == 0` while the rate path below required `window_calls >= min_window_calls`. Fix: hoist `raw_delta = cum_calls - base_raw_calls` and require `raw_delta >= min_window_calls` for the flag.
