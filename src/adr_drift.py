...
#   * 2026-07-24 (#10411): the review-path (review_advisor + review_phase/_phase)
#     is bare-cited as a dependency pointer by ADR-0059/0094/0095/0102 and
#     others — 3 PRs alone spawned ~15 false-positive drift flags. Also see
#     :data:`_shared_infra_fanout_threshold`-based auto-suppression below,
#     which derives this same suppression from citation fan-out instead of a
...
_SHARED_INFRA_MODULES = frozenset(
    {
        ...
        # Review-path — bare-cited as a dependency pointer by ADR-0059/0094/
        # 0095/0102 and others; the next-highest-churn shared-infra source of
        # ADR-drift false positives after #9397 (#10411).
...
def _bare_citation_fanout(path: str, adrs: Iterable[ADR]) -> int:
    """Count how many of *adrs* cite *path* bare (empty symbol set).
    ...
    gates, so only bare citations should count toward the threshold.
    """
...
def _citation_drifts(
    ...
    * **Bare-file citation** (empty cited-symbol set) → drifts on *any*
      ...
      *fanout_threshold* live ADRs (#10411) — a dynamically-derived
      equivalent of the manual allowlist that doesn't need a hand-edit
      for the next high-churn shared module. ``fanout_threshold=None``
      disables this derived suppression (only the manual allowlist
      applies).
...
def compute_drift(
    ...
    *,
    ...
    ``shared_infra_fanout_threshold`` (#10411): when given, a ``src/`` path
    bare-cited by at least this many live (Accepted/Proposed) ADRs is
    treated as shared infra — same suppression as :data:`_SHARED_INFRA_MODULES`
    but derived from citation fan-out instead of a hand-maintained list.
    ``None`` (the default) disables this derived suppression, preserving
...
def compute_drift_by_adr(
    ...
    *,
    ...
    :func:`compute_drift` call — see its docstring (#10411).
    """
