...
class ADRCouncilReviewer:
    ...
    async def _handle_pre_review_failure(
        ...
        hitl_number = await self._prs.create_issue(
            title, body, labels=list(self._config.hitl_label)
        )
        if hitl_number <= 0:
            logger.warning(
                "ADR-%04d pre-review HITL issue creation returned invalid number (%s)",
                validation.adr_number,
                hitl_number,
            )
