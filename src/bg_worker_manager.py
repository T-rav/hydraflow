...
from worker_catalog import default_worker_interval
...
class BGWorkerManager:
    ...
    def get_interval(self, name: str) -> int:
        ...
        loop = self._bg_loop_registry.get(name)
        if loop is not None:
            return loop._get_default_interval()  # noqa: SLF001 - manager owns loop registry.
        return default_worker_interval(self._config, name) or self._config.poll_interval
