...
# Visual-validation + visual merge-gate methods, extracted verbatim to
# ``_visual_gate.py`` (#10840 god-file concentration work). ``ReviewPhase``
# inherits the mixin so the public surface is unchanged — every moved method
# (``check_visual_gate``, ``escalate_visual_failure``, ...) still resolves as
# an attribute of ``ReviewPhase`` exactly as before.
...
class ReviewPhase(VisualGateMixin):
