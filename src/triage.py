...
Separately from clarity, rate the *implementation* complexity on a 0-10 scale
(`"complexity_score"`). This gates how much planning/review ceremony the
issue receives, so score the work, not the writing:
- **0-2**: Trivial — copy/config/label changes, one-line fixes
- **3-5**: Small bounded change — one or two files, existing patterns, low risk
- **6-8**: Substantial — multi-file feature, new tests/wiring, moderate risk
- **9-10**: Cross-cutting — architectural change, migrations, epics, program-scale work

Always include `"complexity_score"` in your response.
...
```json
{{"ready": true, "reasons": [], "issue_type": "feature", "clarity_score": 9, "complexity_score": 4, "needs_discovery": false, "enrichment": "## Triage Enrichment\\n\\n**Interpreted intent:** ...\\n**Affected area:** ...\\n**Acceptance criteria:**\\n- ...", "already_addressed": false, "claim_verified": null}}
```

or for vague product ideas needing discovery:

```json
{{"ready": true, "reasons": [], "issue_type": "feature", "clarity_score": 3, "complexity_score": 8, "needs_discovery": true, "enrichment": ""}}
```

or for truly insufficient issues:

```json
{{"ready": false, "reasons": ["Specific reason why this cannot proceed"], "issue_type": "bug", "clarity_score": 0, "complexity_score": 5, "needs_discovery": false, "enrichment": "", "already_addressed": false, "claim_verified": null}}
```

or for a claim that is demonstrably false / already implemented (the described problem no longer exists at HEAD):

```json
{{"ready": false, "reasons": ["Claim not reproducible: <evidence>"], "issue_type": "bug", "clarity_score": 5, "complexity_score": 3, "needs_discovery": false, "enrichment": "", "already_addressed": true, "claim_verified": false}}
```
"""
    ...
    def _result_from_dict(data: dict[str, object], issue_number: int) -> TriageResult:
        ...
        # Absent/invalid complexity is None, never 0 (#11298 audit): 0 means
        # "trivially simple" and would throw the plan-tier gates wide open,
        # while epic decomposition keys on high scores. Each consumer picks
        # its own conservative default for None.
        complexity = data.get("complexity_score")
        score = int(complexity) if isinstance(complexity, int | float) else None
        ...
        return TriageResult(
            ...
            complexity_score=max(0, min(score, 10)) if score is not None else None,
