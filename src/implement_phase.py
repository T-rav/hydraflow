...
class ImplementPhase:
    ...
    async def _handle_implementation_result(
        ...
        # Null-delivery guard (issue #9480): a non-empty diff whose every file
        # is a planner diagram or auto-generated artifact (no code/tests/assets)
        # is not a real implementation. The planner copies .likec4 context
        # diagrams into the worktree; when the agent produces no code the
        # auto-commit fallback salvages only those diagrams, fooling the
        # zero-commit guard (commits >= 1). Treat it as a failed attempt — do
        # not push or open a PR that would falsely close the issue on merge.
        if await self._is_null_delivery(result):
