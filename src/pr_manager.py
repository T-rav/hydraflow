...
# Re-export for backward compatibility. PRManagerPromotionMixin carries the
# promotion/RC methods extracted to pr_manager_promotion.py; PRManager inherits
# it, so all previously-defined methods remain reachable unchanged.
__all__ = [
    "CommentFormatter",
    "SelfReviewError",
    "PRManager",
...
class PRManager(PRManagerPromotionMixin):
    """Pushes branches, creates PRs, merges, and manages labels.

    The promotion / RC branch lifecycle sub-workflow (ADR-0042) lives in
    :class:`pr_manager_promotion.PRManagerPromotionMixin`; it is part of this
    class via inheritance, so the public surface is unchanged.
    """
    ...
    async def merge_pr(self, pr_number: int, *, auto_rebase: bool = False) -> bool:
        ...
        for attempt in range(2):
            ...
            # (the canonical false_close.CLOSE_KEYWORD_RE, #11481) so an
            # embedded reference like "Fixed #12: address #34" picks the issue
            # the PR actually fixes (not the first bare "#N"); fall back to
            # the first "#N" for keyword-less titles (e.g. "feat(x): do thing
            # (#123)") so cost is still attributed.
