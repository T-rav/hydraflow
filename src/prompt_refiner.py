...
_LENGTH_PROBE_EXTRA_KWARGS: dict[str, dict[str, str]] = {
    ...
    # scope-check needs a File Delta in the `MODIFIED:` form
    # `parse_file_delta` actually reads. Without one it renders the short
    # `_(none extracted)_` variant rather than the classifier the ±30% gate
    # should be measuring — not an empty render, but not the real prompt
    # either. A bullet list (as plan-compliance uses above) parses to zero
    # files and lands in the same place.
