...
class EpicManager:
    ...
    async def sweep_completed_epics(self) -> dict[str, int]:
        """Auto-close open epics whose state/body sub-issue refs are all closed."""
        epics = await self._fetcher.fetch_issues_by_labels(
            self._config.epic_label, limit=50
        )
        if len(epics) == 50:
            logger.warning(
                "Epic monitor fetched exactly 50 epics for completion sweep — result "
                "may be truncated; some epics may not be swept this cycle"
            )
        ...
        for epic in epics:
            try:
                sub_issues = self._collect_sweep_sub_issues(epic.number, epic.body)
                if not sub_issues:
                    continue
                checked += 1
    ...
    def _collect_sweep_sub_issues(self, epic_number: int, body: str) -> list[int]:
    ...
    async def _try_sweep_epic(
        ...
        """Close the epic if every referenced sub-issue is already closed."""
        fixed_label = self._config.fixed_label[0] if self._config.fixed_label else ""
        ...
        for issue_number in sub_issues:
            ...
            if issue is None:
                ...
                return False
            state_value = getattr(issue.state, "value", issue.state)
            if state_value != GitHubIssueState.CLOSED.value:
                return False
