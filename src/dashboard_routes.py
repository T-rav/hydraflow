...
# ---------------------------------------------------------------------------
# Extracted helpers — formerly nested inside create_router but independent
# of closure-captured state.  Moved to module level to reduce the size of
# create_router and improve testability.
# ---------------------------------------------------------------------------
...
def _new_issue_history_entry(issue_number: int, repo_slug: str) -> dict[str, Any]:
    ...
    elif slug.startswith("http://github.com/"):
        slug = slug[len("http://github.com/") :]
    slug = slug.strip("/")
    issue_url = f"https://github.com/{slug}/issues/{issue_number}" if slug else ""
    return {
...
def _filter_rows_to_items(
    ...
    for row in issue_rows.values():
        ...
        items.append(_build_issue_history_entry(row, state.get_outcome(issue_number)))
    return items
...
def create_router(
    ...
    *config*, *state*, *event_bus*, and *get_orchestrator*) are used for
    ...
    async def _apply_enrichment_and_crate_titles(
        ...
        if enrich_candidates:
            ...
            items = _filter_rows_to_items(
                issue_rows, requested_status, query_text, state
            )
    ...
    async def get_issue_history(
        ...
        items = _filter_rows_to_items(issue_rows, requested_status, query_text, state)
