...
class PlanReviewer(BaseRunner):
    ...
    def _build_prompt(cls, task: Task, plan: str) -> str:
        ...
        return (
            ...
            f"A concrete defect with a concrete fix is worth more than vague "
            f"doubt or praise.\n\n"
            f"First, find the **load-bearing claim** — the one assumption the "
            f"plan is organized around (an abstraction is right, an invariant "
            f"holds, an existing API behaves a certain way) — and try to "
            f"falsify it. If that claim is wrong, the plan is wrong, and that "
            f"is your most important finding.\n\n"
            f"**Verify, do not trust.** You have Bash. When the plan asserts "
            f"something about existing code, an ADR, or a test — that a "
            f"function does X, that a file is shaped a certain way, that a "
            f"convention exists — check it: grep the file, read the ADR, run "
            f"the one relevant test. A plan that cites code or behaviour that "
            f"does not exist as described is a critical finding, not a "
            f"stylistic one. Do not flag a claim you have not checked as if "
            f"you had; an unverified guess dressed as a defect wastes a lap.\n\n"
            f"Review through three lenses, mapping each to the dimensions "
            f"below:\n"
            f"- **Correctness & design** (correctness, edge_cases, "
            f"reproduction) — will it actually work; is the abstraction right "
            f"or is there a materially simpler design; does it match the code "
            f"it touches?\n"
            f"- **Scope & blast radius** (scope_creep, convention) — is this "
            f"the right, minimal change; what existing behaviour does it put "
            f"at risk; is it reversible?\n"
            ...
            f"```\n"
            f"List your single most important finding first — the one "
            f"thing that most needs fixing before this plan is built. The "
            f"Suggestion line is optional but encouraged. If you have no "
            f"findings, emit an empty markers block — that's a clean review."
        )
