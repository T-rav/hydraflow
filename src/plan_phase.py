...
class PlanPhase:
    ...
    async def _handle_plan_success(self, issue: Task, result: PlanResult) -> None:
        ...
        # Unresolved-design-decision gate (#10659). If plan review accumulated
        # >= K design-decision-class CRITICAL concerns (unvalidated core
        # mechanism / needs human sign-off — NOT implementer-addressable
        # buildability or coverage gaps), route to `human-required` instead of
        # swapping to ready. Forwarding these to implement causes the build
        # agent to hang the full timeout and retry-thrash on an underspecified
        # feature. Runs AFTER all adversarial stages have persisted their
        # concerns, so it sees the complete set.
