...
export const PIPELINE_STAGES = [
  ...
  { key: 'hitl',      label: 'Needs Human', color: theme.red,       subtleColor: theme.redSubtle,     role: null,           configKey: null,           track: 'engineering', conditional: true },
...
export const BACKGROUND_WORKERS = [
  ...
  { key: 'adr_reviewer',   label: 'ADR Reviewer',   description: 'Reviews proposed ADRs via a 3-judge panel and routes to accept, reject, or escalate.', color: theme.accent, group: 'intake', tags: ['review'] },
  ...
  { key: 'charter_loop_worker', label: 'Charter Loop Worker', description: "Runs a repo's charter-declared loops (charter.yaml schema_version 2): selects due loops from their cron clauses, dispatches one brokered worker per loop inside its budget/timeout envelope, and receipts every decision including skips. Ships disabled — arming it is an operator ENACT. See ADR-0145.", color: theme.purple, system: true, group: 'governance', tags: ['audit', 'drift'] },
  { key: 'charter_drift_caretaker', label: 'Charter Drift Caretaker', description: "Audits each managed repo's live state against its charter.yaml (declared standards / required artifacts / template layers / coverage floor / domain gate scripts) and files deduped drift issues. See ADR-0121, ADR-0143.", color: theme.purple, system: true, group: 'governance', tags: ['audit', 'drift'] },
  ...
  { key: 'convergence_oscillation', label: 'Thrash Detector', term: 'Convergence Oscillation', description: 'Scans issue convergence ledgers for cross-boundary oscillation (repeated LOOP_BACK across triage/shape/plan or recurring review-lap findings) and escalates stuck issues to HITL, once each. See ADR-0098.', color: theme.purple, group: 'autonomy', tags: ['recovery', 'autonomy'] },
