...
- restricted-mode hardening (ADR-0092): the auto-agent runs on
...
# Spec §5.2 / ADR-0092 — tools the auto-agent must NOT use.
#
...
class AutoAgentRunner(BaseSubprocessRunner[PreflightSpawn]):
    ...
    def _pre_spawn_hook(self, prompt: str) -> None:
        ...
        if self._config.implementation_tool not in ("claude", "codex"):
            logger.warning(
                "auto-agent: restricted-mode tool hardening (ADR-0092) is only "
