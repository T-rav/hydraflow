...
- **Never commit to `main`.** The branch is protected; all changes go through a worktree branch and a pull request. No exceptions, not even for one-line fixes. Look up "Worktree" in [`docs/wiki/gotchas.md`](docs/wiki/gotchas.md). **Create worktrees with [`scripts/hf_worktree.sh`](scripts/hf_worktree.sh) `<dir> <branch>`** (or `make worktree DIR=<dir> BRANCH=<branch>`) — never a bare `git worktree add`: when the directory already exists, `add` fails but a chained `cd`/`merge` still runs against the stale branch (#11501).
