...
- **Never commit to `main`.** The branch is protected; all changes go through a worktree branch and a pull request. No exceptions, not even for one-line fixes. Create worktrees with `scripts/hf_worktree.sh <dir> <branch>` (or `make worktree DIR=<dir> BRANCH=<branch>`), never a bare `git worktree add` — a bare `git worktree add` fails when `<dir>` already exists, but the failure is easy to miss and the next chained command (`cd`, `git merge`) silently succeeds against whatever branch the stale directory was actually on. Look up "Worktree" in [`docs/wiki/gotchas.md`](docs/wiki/gotchas.md).
...
- **Always verify branch identity after creating a worktree** with `git rev-parse --abbrev-ref HEAD` inside it, before editing, merging, or committing. Same class of mistake as the rule above: never trust that a step did what it said. `scripts/hf_worktree.sh` already prints the resolved branch on success — this rule is the backstop for the rare case where a worktree was created by hand instead.
...
- **A finding whose needle is a pattern (regex/AST shape recurring at multiple sites) sweeps all sites and files ONE class issue, folding later siblings into it** — never one issue per site. Run `python scripts/find_class_check.py --check --source ... --needle ... --title ...` before filing (see [`.claude/commands/hf.issue.md`](.claude/commands/hf.issue.md) Phase 3); the matching engine is [`src/find_class_key.py`](src/find_class_key.py). Board-growth analysis found three defect families each spawning 3-4 sibling issues for one class (#11292) before this existed.
