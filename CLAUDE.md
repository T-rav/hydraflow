...
- **Never include line numbers in ADR source citations.** In `## Related` sections, cite source files by function or class name only (e.g., `src/config.py:_resolve_paths`). Do NOT add `(line 42)` or similar — line numbers drift as the source file is edited and council reviews will flag them as stale.
