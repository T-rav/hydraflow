...
def _make_loop_with_fake_github(
    ...
    Exercises the production `_verify_issue` gh CLI path (`issue view` /
    `issue edit`) against in-memory fake state instead of a stubbed
    `_run_gh`, so the repair path's reads and writes are observable (#11246).
    """
...
class TestVerifyIssueAgainstFakeGitHub:
    ...
    The fake's `gh issue view` used to return ``{"comments": []}`` for every
    ``--json`` selector, so the loop always saw no labels and an empty body
    and fired the label/screenshot repair unconditionally.
    """
