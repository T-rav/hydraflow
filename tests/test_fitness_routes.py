...
def test_latest_fitness_per_worker(tmp_path) -> None:
    ...
    old = LoopFitness(
        worker_name="alpha", kind=FitnessKind.SCORED, score=0.1, sample_count=5,
        confidence=Confidence.OK, timestamp=datetime(2026, 6, 1, tzinfo=UTC),
    )
    new = LoopFitness(
        worker_name="alpha", kind=FitnessKind.SCORED, score=0.9, sample_count=9,
        confidence=Confidence.OK, timestamp=datetime(2026, 6, 30, tzinfo=UTC),
    )
