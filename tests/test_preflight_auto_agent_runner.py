...
async def test_run_builds_restricted_claude_command(tmp_path: Path) -> None:
    """ADR-0082 / W7-OC-1: the auto-agent runs on attacker-reachable input, so
...
async def test_run_unrestricted_escape_hatch_reverts_to_bypass(tmp_path: Path) -> None:
    """agent_unrestricted_tools=True is the ADR-0082 escape hatch: it reverts
    ...
    # the ADR-0082 escape hatch, so set the field via model_copy on the built
