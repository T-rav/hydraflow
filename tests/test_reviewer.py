...
async def test_fix_ci_populates_files_changed(
    ...
    with (
        ...
        patch.object(
            runner,
            "_get_changed_files",
            AsyncMock(return_value=["src/ci_fix.py", "tests/test_ci.py"]),
        ),
        patch.object(runner, "_has_changes", AsyncMock(return_value=True)),
        patch.object(runner, "_get_commit_stat", AsyncMock(return_value="")),
        patch.object(runner, "_save_transcript"),
    ):
        result = await runner.fix_ci(pr_info, task, tmp_path, "Failed: lint", attempt=1)

    assert result.files_changed == ["src/ci_fix.py", "tests/test_ci.py"]
