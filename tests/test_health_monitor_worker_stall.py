...
def test_config_defaults_opt_in_staging_bisect() -> None:
    """staging_bisect, flake_tracker, skill_prompt_eval ship opted-in (tight=2)."""
    cfg = HydraFlowConfig(repo="hydra/hydraflow")
    assert cfg.worker_stall_tight_loops == [
        "staging_bisect",
        "flake_tracker",
        "skill_prompt_eval",
    ]
...
def test_multiplier_helper_tight_for_opt_in_blanket_otherwise(hm_env) -> None:
    ...
    assert hm._worker_stall_multiplier("skill_prompt_eval") == 2
