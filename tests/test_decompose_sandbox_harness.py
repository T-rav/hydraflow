...
- ``DecompositionCouncil._execute_council`` short-circuits to a scripted
...
class TestExecuteCouncilSentinel:
    async def test_sentinel_short_circuits_to_scripted_reply(self, monkeypatch):
        ...
        council = DecompositionCouncil(runner=object(), config=ConfigFactory.create())
        fake = FakeLLM()
        fake.script_decomposition(7, ["DIR-REPLY", "VAL-REPLY"])
        council._mockworld_fake_llm = fake  # attached by sandbox_main in production

        assert (
            await council._execute_council("direction prompt", issue_number=7)
            == "DIR-REPLY"
        )
        assert (
            await council._execute_council("validation prompt", issue_number=7)
            == "VAL-REPLY"
        )
        # Drained → None (the council treats None as a garbled/retryable pass).
        assert await council._execute_council("p", issue_number=7) is None
    ...
    async def test_no_sentinel_uses_real_seam(self, monkeypatch):
        ...
        council = DecompositionCouncil(runner=object(), config=ConfigFactory.create())
        out = await council._execute_council("p", issue_number=7)
        assert calls["n"] == 1
