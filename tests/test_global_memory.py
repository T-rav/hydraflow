...
class TestLoadGlobalDigest:
    def test_returns_empty_when_file_missing(self, tmp_path: Path) -> None:
        store = _make_store(tmp_path)
        assert store.load_global_digest() == ""

    def test_returns_empty_when_directory_missing(self, tmp_path: Path) -> None:
        store = GlobalMemoryStore(tmp_path / "nonexistent" / "global_memory")
        assert store.load_global_digest() == ""

    def test_loads_existing_digest(self, tmp_path: Path) -> None:
        store = _make_store(tmp_path)
        _write_digest(tmp_path, "## Global\n\n- Always use type hints")
        result = store.load_global_digest()
        assert "Always use type hints" in result

    def test_strips_whitespace(self, tmp_path: Path) -> None:
        store = _make_store(tmp_path)
        _write_digest(tmp_path, "   \n  content  \n   ")
        assert store.load_global_digest() == "content"
...
# ---------------------------------------------------------------------------
# TestGetCombinedDigest
# ---------------------------------------------------------------------------


class TestGetCombinedDigest:
    def test_returns_local_when_no_global(self, tmp_path: Path) -> None:
        store = _make_store(tmp_path)
        result = store.get_combined_digest("local content", max_chars=1000)
        assert result == "local content"

    def test_returns_global_when_no_local(self, tmp_path: Path) -> None:
        store = _make_store(tmp_path)
        _write_digest(tmp_path, "global content")
        result = store.get_combined_digest("", max_chars=1000)
        assert "global content" in result

    def test_returns_empty_when_both_empty(self, tmp_path: Path) -> None:
        store = _make_store(tmp_path)
        result = store.get_combined_digest("", max_chars=1000)
        assert result == ""

    def test_combines_global_and_local(self, tmp_path: Path) -> None:
        store = _make_store(tmp_path)
        _write_digest(tmp_path, "global baseline knowledge")
        result = store.get_combined_digest("local override", max_chars=500)
        assert "global baseline knowledge" in result
        assert "local override" in result

    def test_local_is_prioritised_when_truncating(self, tmp_path: Path) -> None:
        store = _make_store(tmp_path)
        global_content = "G" * 500
        local_content = "L" * 500
        _write_digest(tmp_path, global_content)
        # max_chars is smaller than combined length
        result = store.get_combined_digest(local_content, max_chars=600)
        # Local must be fully present (or dominate when truncated to max_chars)
        assert result.endswith(local_content) or local_content in result

    def test_result_never_exceeds_max_chars(self, tmp_path: Path) -> None:
        store = _make_store(tmp_path)
        _write_digest(tmp_path, "G" * 800)
        local = "L" * 800
        result = store.get_combined_digest(local, max_chars=400)
        assert len(result) <= 400

    def test_truncates_local_to_max_chars_when_no_global(self, tmp_path: Path) -> None:
        store = _make_store(tmp_path)
        result = store.get_combined_digest("x" * 2000, max_chars=100)
        assert len(result) == 100

    def test_truncates_global_to_max_chars_when_no_local(self, tmp_path: Path) -> None:
        store = _make_store(tmp_path)
        _write_digest(tmp_path, "g" * 2000)
        result = store.get_combined_digest("", max_chars=100)
        assert len(result) == 100
