...
Ratchet: one pre-existing collision (``s50_convergence_review`` +
``s50_disturbance_dampener_idle_poll``) is grandfathered. ``s50_convergence_review``
is an ADR-0094/0095 conformance anchor, so renumbering is a scoped follow-up
(tracked separately) rather than part of this guard. The grandfather set is
frozen: a NEW collision fails the test, and if the s50 collision is ever
resolved this test fails too, forcing the stale exception out (self-retiring).
"""
...
_GRANDFATHERED_COLLISIONS = frozenset({"s50"})
...
def test_grandfathered_collisions_are_not_stale() -> None:
    ...
    Keeps the exception list honest: when the s50 collision is renumbered away,
    this fails and forces removal of the now-stale grandfather entry.
    """
