...
def test_the_count_survives_the_hitl_handoff() -> None:
    ...
    state = _tracker()
    for _ in range(3):
        state.bump_plan_validation_rejections(11544)

    state.clear_adversarial_state(11544)

    assert state.get_plan_validation_rejections(11544) == 3, (
...
def test_the_count_survives_a_reload_from_disk() -> None:
    ...
    assert reloaded.get_plan_validation_rejections(11544) == 2
...
def test_the_count_is_per_issue() -> None:
    ...
    state.bump_plan_validation_rejections(11544)
    state.bump_plan_validation_rejections(11544)
    state.bump_plan_validation_rejections(11795)

    assert state.get_plan_validation_rejections(11544) == 2
    assert state.get_plan_validation_rejections(11795) == 1
