...
Mutation-proven: with ``venvPath``/``venv`` removed from ``pyproject.toml``,
``BaseModel`` reveals as ``Unknown`` and the attribute error vanishes, so
``test_bare_pyright_reports_the_pydantic_attribute_typo`` fails.
"""
