...
# ---------------------------------------------------------------------------
# Extracted helper: _build_plan_section
# ---------------------------------------------------------------------------


class TestBuildPlanSection:
    """Tests for the extracted _build_plan_section helper."""

    def test_returns_empty_when_no_plan(self, config, event_bus: EventBus) -> None:
        """Should return empty string when no plan is found anywhere."""
        config.repo_root.mkdir(parents=True, exist_ok=True)
        runner = AgentRunner(config, event_bus)
        issue = TaskFactory.create(id=999, comments=[])
        from prompt_builder import PromptBuilder

        builder = PromptBuilder()
        with patch("agent.logger"):
            result = runner._build_plan_section(issue, builder)
        assert result == ""

    def test_returns_plan_section_from_comment(
        self, config, event_bus: EventBus
    ) -> None:
        """Should extract plan from comments and format as Implementation Plan."""
        runner = AgentRunner(config, event_bus)
        plan_comment = "## Implementation Plan\n\nStep 1: Do this\n\n---\n*Generated by HydraFlow Planner*"
        issue = TaskFactory.create(comments=[plan_comment])
        from prompt_builder import PromptBuilder

        builder = PromptBuilder()
        result = runner._build_plan_section(issue, builder)
        assert "Follow this plan closely" in result
        assert "Step 1: Do this" in result


# ---------------------------------------------------------------------------
# Extracted helper: _build_review_feedback_section
# ---------------------------------------------------------------------------


class TestBuildReviewFeedbackSection:
    """Tests for the extracted _build_review_feedback_section helper."""
    ...
    def test_returns_section_with_feedback(self, config, event_bus: EventBus) -> None:
        runner = AgentRunner(config, event_bus)
        from prompt_builder import PromptBuilder
...
# ---------------------------------------------------------------------------
# Extracted helper: _build_prior_failure_section
# ---------------------------------------------------------------------------


class TestBuildPriorFailureSection:
    """Tests for the extracted _build_prior_failure_section helper."""
    ...
    def test_returns_section_with_failure(self, config, event_bus: EventBus) -> None:
        runner = AgentRunner(config, event_bus)
        from prompt_builder import PromptBuilder

        builder = PromptBuilder()
        result = runner._build_prior_failure_section("TypeError: x is not y", builder)
        assert "## Prior Attempt Failure" in result
        assert "TypeError: x is not y" in result


# ---------------------------------------------------------------------------
# Extracted helper: _build_feedback_and_escalation_sections
# ---------------------------------------------------------------------------


class TestBuildFeedbackAndEscalationSections:
    """Tests for the extracted _build_feedback_and_escalation_sections helper."""

    def test_returns_empty_when_no_data(self, config, event_bus: EventBus) -> None:
        runner = AgentRunner(config, event_bus)
        from prompt_builder import PromptBuilder

        builder = PromptBuilder()
        feedback, escalation, escalations = (
            runner._build_feedback_and_escalation_sections(builder)
        )
        assert feedback == ""
        assert escalation == ""
        assert escalations == []
