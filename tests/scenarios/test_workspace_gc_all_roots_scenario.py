...
KEEPING same-root worktrees that are dirty or belong to an open issue with
unmerged commits. GitHub state is served by the real ``FakeGitHub`` PRPort so
the safe-to-gc policy runs unchanged; local git is simulated via a
command-aware ``run_subprocess`` fake that records reap operations.
"""
...
class TestWorkspaceGCAllRootsScenario:
    async def test_reaps_closed_fix_worktree_keeps_dirty_and_unmerged(
        ...
        revlist_map = {str(wt_open.resolve()): "3"}  # open issue has unmerged work
        removed: list[str] = []
        ...
        # KEEP: dirty worktree and open-issue-with-unmerged-commits worktree,
        # plus the primary worktree, are never touched.
