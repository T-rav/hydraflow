...
class TestL23StaleIssueLoop:
    ...
    Uses Pattern B (direct instantiation) so ``list_all_issues`` failures
    can be scripted per-test via ``side_effect`` — plain MagicMock, not
    FakeGitHub, since #11418 kept the Port-method contract but this class
    predates seeding via ``add_issue`` for this exact shape.
    """

    def _make_prs_mock(
        self, issues: list[dict], *, fail_fetch: bool = False
    ) -> MagicMock:
        """Return a MagicMock prs with list_all_issues/close_issue configured."""
        prs = MagicMock()
        if fail_fetch:
            prs.list_all_issues = AsyncMock(side_effect=RuntimeError("gh failed"))
        else:
            prs.list_all_issues = AsyncMock(return_value=issues)
...
# Uses Pattern A (real FakeGitHub) since #11418: list_branch_refs /
# list_branch_commits / list_all_issues are now first-class PRPort methods
# implemented by FakeGitHub, so the reconciler's coverage is enforced by the
# Protocol (tests/test_ports.py) rather than a hand-rolled _run_gh dispatcher.
# ---------------------------------------------------------------------------

_BRANCH_GC_REPO = "test-org/test-repo"
_BRANCH_GC_BRANCH = "agent/issue-9553"
_BRANCH_GC_ISSUE = 9553
...
class TestL23cBranchGC:
    ...
    def _make_fake(
        ...
        *,
        ...
        # footgun: a hardcoded date silently crosses a now()-relative
        # threshold).
        fake.add_issue(
            _BRANCH_GC_ISSUE,
            "some issue",
        ...
        if issue_state != "OPEN":
            fake._issues[_BRANCH_GC_ISSUE].state = "closed"
        if has_open_pr:
            fake.add_pr(number=42, issue_number=_BRANCH_GC_ISSUE, branch=branch)
        return fake
    ...
    async def test_delete_enabled_and_already_commented_deletes_branch(self, tmp_path):
        ...
        fake = self._make_fake(commit_age_days=30)
        ...
        assert _BRANCH_GC_BRANCH not in fake._branch_commits
    ...
    async def test_open_pr_skips_entirely(self, tmp_path):
        ...
        fake = self._make_fake(has_open_pr=True)
        loop = self._make_loop(tmp_path, fake)
    ...
    async def test_young_branch_skips(self, tmp_path):
        ...
        fake = self._make_fake(commit_age_days=0)
        loop = self._make_loop(tmp_path, fake)
