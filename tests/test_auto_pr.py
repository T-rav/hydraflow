...
# ---------------------------------------------------------------------------
# Direct-merge fallback helpers (#12053)
# ---------------------------------------------------------------------------

_FALLBACK_PR = "https://github.com/x/y/pull/12"
# The stderr GitHub returns when the repo-level "Allow auto-merge" setting is
# off — the failure mode the fallback exists for.
_AUTO_DISALLOWED = (
...
def _merge_gh_stub(calls: list[list[str]], *, rc_by_auto: dict[bool, int]):
    """Fake ``_run_gh`` whose exit code depends on whether ``--auto`` is present."""

    def fake_gh(cmd: list[str], **_: object) -> subprocess.CompletedProcess[str]:
        calls.append(cmd)
        rc = rc_by_auto["--auto" in cmd]
        return subprocess.CompletedProcess(
            cmd, rc, stdout="", stderr=_AUTO_DISALLOWED if rc else ""
        )

    return fake_gh


def test_merge_pr_best_effort_retries_direct_squash_when_auto_is_disallowed(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    from auto_pr import _merge_pr_best_effort

    calls: list[list[str]] = []
    monkeypatch.setattr(
        "auto_pr._run_gh", _merge_gh_stub(calls, rc_by_auto={True: 1, False: 0})
    )

    _merge_pr_best_effort(_FALLBACK_PR, worktree_path=tmp_path)

    assert calls == [
        ["gh", "pr", "merge", _FALLBACK_PR, "--auto", "--squash"],
        ["gh", "pr", "merge", _FALLBACK_PR, "--squash"],
    ]


def test_merge_pr_best_effort_stops_after_a_successful_arm(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    from auto_pr import _merge_pr_best_effort
    ...
    calls: list[list[str]] = []
    monkeypatch.setattr(
        "auto_pr._run_gh", _merge_gh_stub(calls, rc_by_auto={True: 0, False: 0})
    )
...
def test_merge_pr_best_effort_never_raises_when_both_attempts_fail(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    from auto_pr import _merge_pr_best_effort
    ...
    calls: list[list[str]] = []
    monkeypatch.setattr(
        "auto_pr._run_gh", _merge_gh_stub(calls, rc_by_auto={True: 1, False: 1})
    )

    # Best-effort contract: an unmergeable bot PR is logged, never raised.
    _merge_pr_best_effort(_FALLBACK_PR, worktree_path=tmp_path)

    assert len(calls) == 2


def _merge_run_subprocess_stub(
    calls: list[tuple[str, ...]], *, fail_when_auto: bool, fail_when_direct: bool
):
    async def fake_run(
        *cmd: str,
        cwd: Path | None = None,
        ...
        del cwd, gh_token, timeout, runner
        calls.append(cmd)
        is_auto = "--auto" in cmd
        if (is_auto and fail_when_auto) or (not is_auto and fail_when_direct):
            raise RuntimeError(_AUTO_DISALLOWED)
        return ""
...
@pytest.mark.asyncio
async def test_merge_pr_best_effort_async_retries_direct_squash_when_auto_is_disallowed(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    from auto_pr import _merge_pr_best_effort_async

    calls: list[tuple[str, ...]] = []
    monkeypatch.setattr(
        "subprocess_util.run_subprocess",
        _merge_run_subprocess_stub(calls, fail_when_auto=True, fail_when_direct=False),
    )

    await _merge_pr_best_effort_async(
        _FALLBACK_PR, worktree_path=tmp_path, gh_token="t"
    )
...
@pytest.mark.asyncio
async def test_merge_pr_best_effort_async_stops_after_a_successful_arm(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    from auto_pr import _merge_pr_best_effort_async
    ...
    monkeypatch.setattr(
        "subprocess_util.run_subprocess",
        _merge_run_subprocess_stub(calls, fail_when_auto=False, fail_when_direct=False),
    )
...
@pytest.mark.asyncio
async def test_merge_pr_best_effort_async_never_raises_when_both_attempts_fail(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    from auto_pr import _merge_pr_best_effort_async
    ...
    monkeypatch.setattr(
        "subprocess_util.run_subprocess",
        _merge_run_subprocess_stub(calls, fail_when_auto=True, fail_when_direct=True),
    )
