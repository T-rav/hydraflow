...
async def test_issue_view_projects_requested_labels_and_body() -> None:
    ...
    hardcoded ``{"comments": []}`` for every selector, so ReportIssueLoop's
    ``_verify_issue`` always saw no labels and an empty body under MockWorld.
    """
    gh = FakeGitHub()
    gh.add_issue(
        42,
...
# body through this exact command; the fake used to fall through to the
# unknown-verb "[]" return, so the repair was never observable in state.
# ---------------------------------------------------------------------------
