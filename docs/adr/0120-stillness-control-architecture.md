...
- **Enforced by (2026-08-13, #10824):** `src/control_register.py` — Ruling 1's classification is machine-readable in `control/fleet.yaml` (key parity with the orchestrator's `bg_loop_registry` guarded by `tests/test_fleet_register_parity.py`); setpoints live in `control/setpoints.yaml` where **an unsigned setpoint is inert** (the envelope-discipline invariant in code). The first conversion is live: `GateHealthLoop` reads its signed setpoint each cycle and quiesces the pass-rate finders in-band (deadband + `SchmittHysteresis` — the first production instantiation of `signal_control`). `fleet_counts()` supplies the registered-instrument census ADR-0133's `widened_sigma_multiplier(n)` migration awaits; `measurement_noise_for()` is the #10825 rung-3 R-join seam.
