# ADR-0079 — ADRReviewerLoop: Autonomous Panel Review for Proposed ADRs
...
to run the review-panel process manually. The `ADRReviewPanel` in
...
the panel review, update status. There is no reason to require human
...
3. Delegates to `ADRReviewPanel.review_proposed_adrs()`, which:
   ...
   - Advances ADR status to Accepted or files a `hydraflow-find` issue for
     panel escalation (per ADR-0040 — Proposed-only filter is the intentional scope).
...
- Proposed ADRs receive automated panel review without operator scheduling.
...
- The loop does not open PRs; ADR status changes land via the
  `ADRReviewPanel` toolchain, which writes files directly.

## Alternatives considered

- **Manual invocation only.** Rejected: the existing `ADRReviewPanel`
...
- `src/adr_reviewer.py:ADRReviewPanel`
