...
- **Harness determinism (#10859).** Deferring to the real `HydraFlowConfig` defaults fixed the fidelity bug above but reopened a reproducibility one: `HydraFlowConfig()` unconditionally applies env-var overrides and reads `repo_root/.env` for git identity, so the same call could render a different corpus depending on the machine's `HYDRAFLOW_*` environment and `.env` file — "same input → same score, always" broken by the host, not the prompt. `_real_config_defaults()` now builds its config through `config.declared_default_config()` instead of a bare `HydraFlowConfig()`: it scrubs every key `config.env_override_keys()` declares (plus any live `HYDRAFLOW_`/`HYDRA_`/`GIT_`-prefixed key, belt-and-braces) from `os.environ` for the duration of construction, and defaults `repo_root` to a freshly created empty temp directory so the git-identity `.env` fallback finds nothing to read — restoring both afterward. `tests/architecture/test_config_env_key_coverage.py` walks the actual `resolve_defaults` call graph and fails the build if a future env read is added without a matching entry in `env_override_keys()`.
