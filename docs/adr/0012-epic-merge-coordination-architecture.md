...
it instead to the epic subsystem (`EpicCompletionChecker._create_release_for_epic()`). ADR-0012 intentionally places
...
release-creation is an epic-level primitive that, since PR #2689, is not attached to
any automatic trigger — neither epic close nor the bundle-merge `release_epic()` action
mints a tag (see ADR-0011, corrected in #11569) — while merge-coordination runs *before*
each individual merge to decide whether to proceed or hold. ADR-0012 does not supersede ADR-0011 (Epic Release Creation Architecture); the two ADRs govern different stages
