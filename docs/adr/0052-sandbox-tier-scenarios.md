...
5. **CI gate (PR C):** every runnable sandbox scenario must pass on the rc/* promotion PR before staging→main merge. Failures auto-dispatch the auto-agent self-fix loop (`SandboxFailureFixerLoop`). Placeholder scenarios are prohibited by ADR-0065.
