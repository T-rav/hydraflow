...
3. **A degraded advisor fails OPEN by default: the approve advances to merge.** If the PostVerifyAdvisor lens judge fails (non-credit, non-bug exception) or the advisor surface is disabled, the judge degrades to approve (`JudgeVerdict(approve=True, reasoning="judge-degraded")`) and `HybridGate` runs with `fail_default_approve=True`, so a reviewer-approved PR merges on advisor degradation. This is the deliberate Phase 1 design decision: the advisor is a fail-open second opinion and the reviewer verdict remains primary. Setting `HYDRAFLOW_REVIEW_POSTVERIFY_FAIL_AS_VETO=true` flips advisor-failure handling to fail-closed (failure treated as veto, loop back). The backstops on the fail-open path are the deterministic check (open code-scanning alerts always block the merge), the outer lap budget plus oscillation detection, and `reraise_on_credit_or_bug(exc)` (credit exhaustion and likely-bug exceptions always propagate rather than degrade). Operators who want a fail-closed merge gate must set `HYDRAFLOW_REVIEW_POSTVERIFY_FAIL_AS_VETO=true`. With the pre-flight advisor surface disabled, blast radius defaults to "low" (one lens pass); operators wanting blast-radius-scaled lens passes need the pre-flight advisor enabled.
