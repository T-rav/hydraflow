...
multi-repo coordination model proposed in ADR-0006 (repo runtime isolation; now superseded).
...
within the current subprocess. The `/api/repos` endpoints (defined in ADR-0007 — dashboard api multi repo scoping)
...
- Multi-repo mode remains opt-in and undertested until integration tests cover
  the registry lifecycle (see ADR-0022 — integration test architecture cross phase).
