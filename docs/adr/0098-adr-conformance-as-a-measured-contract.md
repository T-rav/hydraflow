...
- `UNRESOLVED` with a confirmed rename (the named check exists under a new identity: an identical test node moved, a target renamed) is re-pointed automatically — the loop opens a tiny PR retargeting `Enforced by:`. No confident match means the guardrail was deleted, not moved, and the outcome is treated as `FAIL`.
...
`AdrConformanceLoop`'s entire write surface is three actions: re-point `Enforced by:` on a confirmed rename, file or update a remediation issue, and file an `adr_reviewer` supersession proposal. It has no code path that edits a check's assertion body or an ADR's `Enforcement:` / `decision-of-record` fields, and a unit test asserts the loop cannot mutate them. The jsonl it persists is gitignored scratch state, not a decision artifact. Nothing in this design lets the loop mark itself green by softening what green means.

### `AdrConformanceLoop` is on the ADR-0046 recursion ladder

`FitnessScorecardLoop` (ADR-0093) is read-only: it observes and mutates nothing, so ADR-0046's "who watches the watcher" question does not apply to it — there is no behavior change for a meta-observer to bound. `AdrConformanceLoop` is different in kind: it **mutates** by filing issues and opening pointer-fix PRs. That makes it a producer in the ADR-0046 sense, and it sits **on** the recursion ladder rather than beside it. It is bounded the same way every other mutating caretaker loop is bounded: a `DedupStore` caps it at one open remediation issue per ADR, a per-ADR attempt budget caps how long it keeps re-filing before escalating to supersession, and `enabled_cb("adr_conformance")` (ADR-0049) is the kill switch that stops it outright. No new recursion-termination mechanism was invented for this loop; it reuses the ladder's existing rungs.
