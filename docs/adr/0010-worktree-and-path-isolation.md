...
2. **State, events, and session files are repo-scoped.** `_resolve_repo_scoped_paths()`
   places `state.json`, `events.jsonl`, and `sessions.jsonl` under
...
- `src/config.py:HydraFlowConfig` — `_resolve_base_paths`, `_resolve_repo_scoped_paths`, `worktree_path_for_issue`,
