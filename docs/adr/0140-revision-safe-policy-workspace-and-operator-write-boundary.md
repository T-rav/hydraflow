...
**The bind is host business; the kill switch is not.** `dashboard_host` describes one socket for the whole dashboard, so it is read from the host config **and only from there** — re-reading it from a repository runtime would report `dashboard-not-loopback` for a socket that *is* loopback, a refusal naming something the operator cannot fix. `gateway_policy_workspace_enabled` is the opposite: a per-repository editable setting, and an operator who turns it off for one repository has to be obeyed *for that repository*. So the route evaluates the host gate first and then checks **only the kill switch** on the resolved repository. The second pass can close the gate; it can never open a shut one. The read plane reports that same per-repository disposition, because an editor rendering the host's gate would offer a save button that always 403s.

**A slug the server cannot resolve is a 404, not somebody else's policy.** `RepoRouteContext.resolve_runtime` deliberately falls back to the host runtime for a repository that is registered but not started, so a read-only dashboard still renders. On the one write route that fallback is a silent retarget: a mutation naming an unstarted repository would land on the *host* repository's snapshot, authorised by the *host* repository's gate. `_workspace` therefore compares the requested slug against the repository it actually resolved and refuses when they differ.
...
- **The feed says whether it has been read, and whether it still is.** A cycle that threw, or any response that was not 2xx, sets an `unavailable` source state, and every empty-state string is gated on a payload having actually landed. Otherwise a dead endpoint renders as "no policy has been written for this repository" — and, worse, as a **`chain verified`** badge on a chain nobody read, which is a positive tamper-evidence claim about bytes that never arrived. `unavailable` outranks `loaded`, because the hook keeps the last-known view model on failure: a feed that succeeded once and then died would otherwise go silent while the panel kept rendering a frozen revision as current. In that state the editor closes, the audit badge degrades to `chain not re-read`, and the notice says so. Aggregate gets its own state for the same reason — reporting `available` beside a matrix and a history that were never fetched is a self-contradictory pair.
- **Rollback is offered only from a chain that verifies.** The write plane already refuses with `audit-chain-broken`; a console that still rendered the buttons would be offering a control it knows will fail.
...
- **One selection, one instance.** `useOperatorSelection` is per-caller `useState` seeded from the URL at mount, so a second instance freezes at whatever the URL said then. The policy feed is therefore owned by the view that already holds the live selection, not by the container: two instances would have rendered the workspace while the feed stayed disabled, and a repository picked after mount would never have reached it.
