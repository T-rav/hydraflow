...
4. **Bounded depth AND fan-out (moderate budget).** Depth alone is not enough: with up to 6 children per split and depth 2, the naive worst case is 6×6 = 36 leaf pipelines from one root. Two bounds:
   - `max_decomposition_depth` (**default 2**; the epic-lineage follow-up #9757 landed, so nested cascade is correct) — a decomposed child that also stalls may split once more, then hits the floor. Depth is tracked per epic/child; `le=5` bounds the chain.
