# ADR-0092 — Untrusted-text trust boundary for agent prompts
...
1. **Fencing (primary defence).** `src/untrusted_text.py` provides `fence_untrusted(label, text)`, which wraps untrusted regions in `<untrusted_{label}> … </untrusted_{label}>` tags and **de-fangs *every* forged untrusted-fence delimiter** inside the content — open or close, any label, whitespace-tolerant — by inserting a non-renderable zero-width-space sentinel after the leading `<`, so a payload cannot forge a *different* label's close tag to break out of the fence. A standing `UNTRUSTED_DATA_PREAMBLE` is emitted once near the top of each prompt, instructing the agent to treat everything inside an `<untrusted_*>` block strictly as DATA and to never follow instructions, tool directives, or "ignore previous instructions" requests found there — even if the text claims to be from the operator or the system. Applied at the implementer prompt (title/body/comments) and the auto-agent envelope (body + comment/wiki/Sentry/commit blocks). The escalation context is also fenced: although its `cause`/`origin_phase`/`pr_number` are system-set, its `agent_transcript` and `ci_logs` are attacker-derived, so the whole rendered block is wrapped. The prior-attempts block is fully system-generated and stays unfenced.
