...
| P8.5 | STRUCTURAL | CLAUDE.md | `.claude/hooks/` includes at least one hook of each enforced kind: PreToolUse, PostToolUse, Stop | Seed from HydraFlow: `hf.block-destructive-git` (PreToolUse), `hf.auto-lint-after-edit` (PostToolUse), `hf.cleanup-code-change-marker` (Stop) |
| P8.6 | STRUCTURAL | src/trace_collector.py | In-process trace collector writes subprocess traces per phase/run | Port `trace_collector.py`; without traces, session retros have nothing to mine |
...
| P10.8 | BEHAVIORAL | docs/standards/testing/README.md | A change ships the test layers its shape requires — the `standard.yaml` matrix decides, keyed on the conventional-commit type; a `fix(...)` PR with no MockWorld scenario fails, an ambiguous `feat(...)` and every `conditional` cell only warn | Per-PR audit gate reuses P10.6's merge-base diff, collects facts via `policy.facts.collect_test_pyramid_facts` and judges them with the `PythonDecisionEngine` — the standard is enforced by the policy engine, not restated in a script |
