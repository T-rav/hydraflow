...
8. **Shared-infra fan-out auto-suppression** (Amended 2026-07-24 by #10411): the manual `src/adr_drift.py:_SHARED_INFRA_MODULES` allowlist (point 3's bare-citation suppression) grew one high-churn module at a time — 3 recent PRs alone spawned ~15 false-positive drift issues from review-path modules (`review_advisor.py`, `review_phase/_phase.py`) that hadn't yet been added. `compute_drift`/`compute_drift_by_adr`/`partition_fleet_drift` now accept an optional `shared_infra_fanout_threshold` (`src/adr_drift.py:_citation_drifts`); when a `src/` path is bare-cited by at least that many live (Accepted/Proposed) ADRs, it is auto-treated as shared infra — same suppression as the manual allowlist, derived from citation fan-out instead of a hand-edit. Config knob `adr_drift_shared_infra_fanout_threshold` (default 4, range 2–100, env `HYDRAFLOW_ADR_DRIFT_SHARED_INFRA_FANOUT_THRESHOLD`); `None` (the pure functions' default) disables it, so callers that don't pass it see unchanged behavior. The auditor threads the config value into both `compute_drift_by_adr` (stale-rollup reconcile) and `partition_fleet_drift` (the main scan). A genuinely symbol-owning citation (`EXAMPLE.py:Bar`) is unaffected by fan-out regardless of how many *other* ADRs bare-cite the same file. Fleet rollups now also persist their member ADR numbers (`src/state/_adr_audit.py:AdrAuditStateMixin.set_adr_rollup`'s optional `adr_numbers`, defaulting to `[]` for per-ADR rollups and pre-existing fleet rollups) — a sibling caretaker loop (`AdrDriftResolverLoop`, #9976, not itself ADR-governed) reads them to triage each fleet member individually and auto-close the batch when every member classifies CONSISTENT, instead of leaving every fleet-batched issue one-shot/human-closed-only.
...
- **#10411 shared-infra fan-out amendment (2026-07-24):** 3 recent PRs (#10376, #10396, #10380) alone spawned ~15 false-positive drift issues, all traced to review-path modules not yet on the manual `_SHARED_INFRA_MODULES` allowlist. Decision point 8 adds churn-derived auto-suppression alongside it (the allowlist is not removed — both suppression paths are OR'd). No state migration: `adr_numbers` is a new optional field on the existing `adr_rollup_issues` state entries, defaulting to `[]` for every rollup persisted before this change.
