...
7. **Fleet batch** (Amended 2026-07-20 by #9662): when a *single* PR drifts at least `adr_drift_fleet_batch_threshold` distinct ADRs (default 4, range 2–100, env `HYDRAFLOW_ADR_DRIFT_FLEET_BATCH_THRESHOLD`), the loop files **one batched rollup** for that PR listing every affected ADR, instead of N per-ADR rollups. The partition is pure (`src/adr_drift.py:partition_fleet_drift` → `src/adr_drift.py:FleetDriftBatch`); a fleet PR's findings are removed from the per-ADR aggregation so the same drift is never double-filed. Below-threshold PRs keep the per-ADR shape of points 3–6 unchanged. Batch keys use the `FLEET-<pr>` discriminator (state key `FLEET-<pr>`, dedup key `adr_touchpoint_auditor:FLEET-<pr>`) — disjoint from the per-ADR `ADR-NNNN` sub-namespace. **Close semantics differ from per-ADR rollups:** a batched rollup is one-shot from *this* loop's own perspective — it is *not* auto-closed when a member ADR's file is later updated (point 5 does not apply). A human can still close it with a one-line explanation (the close comment is the audit trail, preserving Rule 4's philosophy without reviving `Skip-ADR:`); the loop's manual-close reconcile then clears the `FLEET-<pr>` state, attempt counter, and dedup key so nothing strands. (See point 8 for the sibling caretaker loop that adds an *automated* close path on top of this.)
8. **Fleet batch triage auto-close** (Amended 2026-07-24 by #10457): fleet rollups now persist their member ADR numbers alongside the existing `issue_number`/`pr_numbers` (`src/state/_adr_audit.py:AdrAuditStateMixin.set_adr_rollup`'s optional `adr_numbers`, defaulting to `[]` for per-ADR rollups and pre-existing fleet rollups — no state migration). A sibling caretaker loop, `AdrDriftResolverLoop` (#9976, not itself ADR-governed), reads `adr_numbers` to triage every member ADR of a batch against the same PR and auto-closes the whole batch (`reason="not planned"`) only when *every* member classifies CONSISTENT — fail-closed: a missing member ADR or any triage-call error leaves the entire batch untouched and retried next tick, never partially resolved. Any non-CONSISTENT member leaves the batch open exactly as point 7 describes (a human still closes it). This narrows, but does not remove, point 7's "human-closed-only" characterization: a batch can now also self-close when the triage evidence is unambiguous, the same way per-ADR rollups already do (point 3's triage-before-escalate path).
...
- **#10457 fleet-triage amendment (2026-07-24):** fleet batches were one-shot/human-closed-only since #9662, which meant cross-cutting false-positive batches (e.g. #10388/#10405/#10406) piled up as manual HITL work with no auto-close path, unlike per-ADR rollups (point 3). Decision point 8 extends `AdrDriftResolverLoop`'s existing CONSISTENT-only triage to fleet batches. No state migration: `adr_numbers` is a new optional field on the existing `adr_rollup_issues` state entries, defaulting to `[]` for every rollup persisted before this change — such rollups remain one-shot exactly as before.
