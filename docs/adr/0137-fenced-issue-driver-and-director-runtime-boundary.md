...
- **Ownership is fenced on identity, not only on epoch.** A request or writer lease belonging to another driver, or to another epoch, is refused with `DRIVER_IDENTITY_MISMATCH`. Epoch alone fences the *same* driver across recovery; it does not fence a misrouted request from a different one.
...
- A new `pending_stage_transition` field is required on the driver state layer before #11535 can enforce C5 — the one piece of state this ADR mandates that does not exist yet.
