...
- **Review cannot be hidden or self-performed.** Every catalog entry that is read-only at `REVIEW` — reviewer, architect, test-adequacy — is marked independent of the implementer, and an independence-fenced role may never hold worktree write scope; `admit_dispatch` returns `SELF_REVIEW_FORBIDDEN` when the request's `requesting_spawn_id` is one of the implementer's spawns. The comparison is on the *spawn* id, not `request_id` — a `request_id` is minted fresh by the director and could never collide with a spawn id, so keying the fence on it would have left it permanently unreachable.

  Two clauses were tightened during P5's review (#11543), both because the fence as first written could not fire. The rule was applied to the **reviewer alone**, which left an implementer's lineage free to request an architect or a test-adequacy reader at `REVIEW` and judge its own work; it is now derived from the catalogue — read-only at `REVIEW` implies independent — so a role added there inherits it or reddens. And an **absent** `requesting_spawn_id` used to admit, on the reading that no parent spawn means a fresh process. Nothing in `src/` writes that field: the only production producer is the director model's own JSON, so the only request the fence could ever refuse was one in which the party being fenced volunteered the value that refused it. A fenced role must now *carry* lineage — `WorkerDispatchRequest` refuses to construct one without it, so a director's reply is discarded as malformed at the contract boundary, and `admit_dispatch` refuses the same shape with `LINEAGE_UNKNOWN` for values that skip validation. That does not make a self-reported lineage trusted; it makes an unstated one loud, which is the half a schema can close.
...
- `src/implement_broker.py`: `CANARY_PHASE`, `FenceBreach`, `FENCE_REJECTIONS`, `WorktreeState`, `worktree_probes`, `worktree_state_from_reads`, `worktree_diff_digest`, `check_worker_fence`, `WriterLeaseRegistry`, `needs_writer_lease`, `UNMEASURED_TOKENS`, `Hibernation`, `hibernation_reason`, `hibernation_refusal`, `implement_canary_repo`, `implement_canary_armed`, `implement_canary_covers`, `implement_roles_for_implement_phase`, `resolve_implement_model`, `writer_lease_for` — the Implement canary's pure decision layer (#11542).
- `src/implement_worker_runner.py`: `ImplementWorkerRunner`, `WorktreeMeasurement`, `build_implement_worker_prompt`, `_spawn_implement_worker` — the one place a fenced writer is started, seam-declared in `SANDBOX_SEAMS`.
- `src/director_dispatch.py`: `CanaryDispatchMixin`, `UNOBSERVED_DIGEST` — the director's actuator half, cut out of `fable_director` when the host class crossed the mass sensor's threshold. It holds no state of its own and declares its one borrowed method under `if TYPE_CHECKING:`, so #11629's MRO-shadowing trap cannot reopen; it sits on the same no-authority decision-path list as the observer it was cut from.
