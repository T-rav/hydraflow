...
The two ends of the canary name a repository in two identity spaces — HydraFlow's dial is the canonical `owner/repo`, while a mint request and a resolved identity both carry the path-safe `owner-repo` — so `settings._repo_slug_allowlist` accepts either and stores the runtime slug. An operator copying the canonical form out of `.env.sample` would otherwise get an allow-list that can never match, and a security control that fails open on a format difference, with no log line and no startup error, is worse than no control.
