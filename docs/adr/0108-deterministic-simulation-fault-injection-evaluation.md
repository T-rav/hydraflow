...
| B5 | **`uuid4` identifiers.** Adjustment / event / retrospective IDs are `uuid4`-derived; `uuid4` reads `os.urandom`, so it is deterministic under the hypervisor and nondeterministic otherwise. These IDs surface in the events and assertions a replay would compare. | `src/health_monitor_loop/_decisions.py` (`adj-{uuid4}`), `src/models.py` (`default_factory` uuid4 IDs), `src/phase_utils.py`, `src/retrospective_queue.py`. | **Trivial–medium.** Thread a seeded ID factory; unnecessary under a true hypervisor. |
