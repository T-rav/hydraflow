...
| # | NAME | What it proves |
|---|------|----------------|
| 7 | _coverage gap_ | WorkspaceGCLoop sandbox coverage is not runnable until the sandbox service starts and reports the `workspace_gc` background worker. |
...
ADR-0083 removed non-working placeholder scenarios from the runnable catalog.
...
- Runs a curated 2-scenario subset: `s01_happy_single_issue`, `s06_kill_switch_via_ui`. These are fast scenarios that collectively cover pipeline and UI control wiring.
