...
Two properties matter for recovery: driver identity is fenced (a request or writer lease belonging to another driver or epoch is refused with `DRIVER_IDENTITY_MISMATCH`), and the sandbox assertion is a **required** argument with no default, so a caller that forgets to thread ADR-0137's S4 result fails loudly rather than dispatching.
