...
## Create worktrees with hf_worktree.sh, never a bare `git worktree add`

`git worktree add <dir> <branch>` fails loudly when `<dir>` already exists — but in a chained shell invocation (`git worktree add ...; cd ...; git merge ...`) the later commands still run against whatever branch the stale directory happened to be checked out to, because the `fatal: already exists` from the first command is easy to miss. Worktree directory names (e.g. under `.claude/worktrees/`) get reused across sessions and nothing sweeps them, so any name an agent or human picks has a real chance of colliding with a stale worktree on an unrelated branch.

Use `scripts/hf_worktree.sh <dir> <branch>` (or `make worktree DIR= BRANCH=`) instead of a bare `git worktree add`: it creates the worktree when `<dir>` is absent, exits 0 silently when `<dir>` already holds the requested branch (idempotent re-entry), and exits non-zero — printing expected vs. actual branch plus the exact `git worktree remove <dir>` command — on any mismatch, including a detached HEAD or a directory that isn't a registered worktree at all. It never deletes or repoints an existing worktree, unlike the factory's disposable per-issue workspaces (`WorkspaceManager._create_workspace` `shutil.rmtree()`s a stale directory before recreating it — `src/workspace.py:492-493`): an agent- or human-created worktree can hold hand-written, uncommitted work that a disposable `issue-<N>` workspace never does.

**Why:** this hit three times in one session (2026-08-20) — one case staged 1469 files onto an unrelated branch via a merge into a stale worktree. None reached a push, but only because the branch was verified by hand afterward with `git rev-parse --abbrev-ref HEAD`; nothing in the workflow forced that check.


```json:entry
{"id":"WORKTREE-SAFE-CREATE-001","source_type":"manual","topic":"gotchas","tags":["worktree","git","isolation","safety","agent-workflow"],"rule":"Create worktrees with scripts/hf_worktree.sh <dir> <branch> (or make worktree DIR= BRANCH=), never a bare git worktree add. It creates when the dir is absent, exits 0 idempotently when the dir already holds the requested branch, and exits non-zero -- printing expected vs actual branch and the exact git worktree remove command -- on any mismatch (different branch, detached HEAD, or a dir that isn't a registered worktree at all). It never deletes or repoints an existing worktree.","anti_pattern":"git worktree add <dir> <branch> chained directly into cd <dir> && git merge/commit -- when <dir> already exists on a different branch, worktree add fails loudly (fatal: already exists) but the chained cd and merge still succeed against the stale branch","code_refs":["scripts/hf_worktree.sh","src/workspace.py:492-493","tests/regressions/test_issue_11501.py"],"source_issue":11501,"added":"2026-08-21"}
```
