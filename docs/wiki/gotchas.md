...
2. **A new subprocess call site needs a sandbox seam.** Any NEW `run_subprocess*` / `stream_claude_process` call site added in a `src/*_loop.py` (or a runner) needs a `mockworld.sandbox_main.SANDBOX_SEAMS` declaration or an injected-fake seam, or the seam-completeness ratchet goes red. (#10155 added `HealthMonitorLoop._repo_probe`'s `git ls-remote` without one.) Beware the ratchet's blind spot: a runner CONSTRUCTED INSIDE A METHOD (not injected) escapes every existing air-gap — the `subprocess_runner=` injection and the `_mockworld_fake_llm` sentinels only cover objects the composition root can reach — so it must be rebound explicitly in `sandbox_main` (`air_gap_runner_sentinels`). #11590: `AutoAgentPreflightLoop._build_spawn_fn` built its `AutoAgentRunner` per attempt and spawned a real `claude` inside the air-gapped container once the light lane defaulted on; the loop module has no lexical spawn call, so the ratchet never saw it.
...
## Worktree creation must verify branch identity — never bare `git worktree add`
...
`run_lightweight_agent` spawned `claude -p <prompt> --model <m>` with no `--output-format json`. Bare-text stdout carries no token usage, so the seam recorded `stats=None`; `PromptTelemetry.record`'s zero-usage guard (a success with zero usage on a non-trivial prompt = swallowed API rejection) then reclassified EVERY completed call as `status=failed`, `usage_status=unavailable`, `usage_anomaly=zero_usage_with_prompt`, `estimated_cost_usd=0`. The live ledger showed 100% of `issue_refinement` (187), `sampled_audit` (63), `sampled_audit_adjudicate` (37) rows this way — and the same for every other claude-CLI one-shot source (`wiki_compilation` 667/667, `term_proposer` 563/563, `triage_honeypot` 589/589 …) — while the identical sources via the zai HTTP backend were `success/available`. The only claude one-shot row ever recorded `success` had `prompt_chars=4`, under the guard's prompt threshold (#11514, #11515, #11516; #11117 was the first sibling, closed by moving the provider dial, not by fixing the path).

**Rule:** a one-shot Claude spawn requests `--output-format json`; `runner_utils._claude_cli_complete` unwraps the `{"type": "result", "result": <text>, "usage": {...}}` envelope (`stream_parser.parse_result_envelope`) so callers still receive the bare reply text and the usage reaches the telemetry row. **Diagnosis signature:** a source whose rows are 100% `failed` + `zero_usage_with_prompt` but carry realistic `duration_seconds` (30–240 s) and `transcript_chars` (1–2k) is a completed-but-unobserved call, not a failing spawn — check the argv for the usage flag before suspecting routing, mint, or the model. A zero-usage streak is a telemetry defect until a real `rc != 0` / stderr proves otherwise.

```json:entry
{"id":"TELEMETRY-ONESHOT-CLI-JSON-ENVELOPE-001","source_type":"manual","topic":"gotchas","tags":["telemetry","prompt-telemetry","run_lightweight_agent","zero-usage","output-format-json","cost-accuracy","flake-triage"],"rule":"One-shot Claude CLI spawns (run_lightweight_agent -> _claude_cli_complete) must request --output-format json and unwrap the result envelope via stream_parser.parse_result_envelope, so callers get the bare reply text and the envelope's usage reaches PromptTelemetry. Diagnosis: 100% failed + usage_anomaly=zero_usage_with_prompt rows with realistic duration_seconds and transcript_chars = completed-but-unobserved calls (a missing usage flag), not failing spawns.","anti_pattern":"Spawning claude -p <prompt> --model <m> with no output format, recording stats=None, and letting the zero-usage guard reclassify every successful non-trivial call as status=failed / $0 — then 'fixing' the blind source by moving its provider dial (#11117) instead of the shared spawn path","code_refs":["src/agent_cli.py:build_lightweight_command","src/runner_utils.py:_claude_cli_complete","src/stream_parser.py:parse_result_envelope","src/prompt_telemetry.py:PromptTelemetry.record","tests/regressions/test_issue_11514_lightweight_cli_usage.py"],"source_issue":11514,"added":"2026-08-21"}
```

## Load JSONL by window, never by row count — a fixed cap is a blind instrument that reports confidently

`token_drift.load_and_check_drift` loaded the newest `DRIFT_LOAD_LIMIT = 5000` rows of `inferences.jsonl` and judged the trailing complete ISO week from them. Live telemetry carried 24,434 rows in ONE week (2026-W25, of 31,631 in the file): the open week alone passed 5,000 rows in ~1.5 days, after which the trailing week had zero loaded rows (`insufficient_data` — the #11442 filing actuator silent in exactly the high-burn weeks it exists for), and a cap landing mid-week mis-sampled the shares into a confident wrong verdict. Same rows: full file `ok; 1 drifting`, newest 5,000 `insufficient_data`. The same cap in `scripts/pin_token_baseline.py` sampled an 8-week pin from whatever the newest rows happened to cover (#11581).

**Rule:** a reader whose question is about a calendar window streams the file (`PromptTelemetry.iter_inferences`) and keeps rows by `timestamp` — `token_drift.load_window_rows` keeps exactly `trailing_complete_weeks(now, windows)`, no row cap, memory bounded by the window. A row-count cap (`load_inferences(limit=…)`) is only right for "most recent activity" views. A window the loader knows it could not cover — an `OSError` mid-stream after rows were already kept, or `audit_retention_days_inference_telemetry` falling inside the oldest week (`RunsGCLoop` prunes strictly older than the floor) — is reported as `TelemetryWindow.truncation`, and the engine withholds the verdict (`insufficient_data`) rather than compute one on a partial window; the pin script refuses to pin. **Detection:** a time-windowed metric that goes `insufficient_data` or suspiciously clean precisely when volume spikes = a count cap upstream; count rows per window in the file before trusting the verdict.
