...
The safety finding is the one to remember even if the cost ever closes: **Rego fails OPEN and `opa check --strict` does not catch it.** A rule referencing an input key no collector emits passes `opa check --strict`, evaluates without error, and returns the un-probed verdict — nothing reddens. The equivalent Python raises `KeyError`. An explicit `required_facts` set mitigates it only for facts someone remembered to list.
