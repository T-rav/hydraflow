...
## Open Policy Agent / Rego was measured and NOT adopted — do not add it back without re-measuring

OPA is **not** a HydraFlow dependency, optional or otherwise, and the absence is a ruling rather than a gap. Pilot #11750 built the thing and measured it: `OpaDecisionEngine` reached exact parity with `PythonDecisionEngine` over all 88 Accepted ADRs and an exhaustive 96-subject synthetic space, in **fewer** lines of Rego than the Python it replaced (0.71×, against a 3× kill threshold), and the cross-standard composition probe was genuinely cheaper in Rego (12 lines vs 23). The pre-registered kill criterion **did not fire** — and the verdict is still not-adopted, on a number the criterion did not ask for: running Rego needed a **237-line Python adapter** (binary discovery, availability probe, error taxonomy, JSON marshalling, subprocess, result parsing) plus an 84-line installer and a 43 MB per-platform binary, against the 84 lines of decision logic being replaced. Break-even is ~22 cross-standard rules; `charter.yaml` declares 8 standards.

The safety finding is the one to remember even if the cost ever closes: **Rego fails OPEN, and the tooling meant to catch that is blind to the shape a fact ledger has.** A rule referencing an input key no collector emits passes `opa check --strict`, evaluates without error, and returns the un-probed verdict — nothing reddens; the equivalent Python raises `KeyError`. OPA's METADATA `schemas:` + `opa check --schema` does catch it, but only on statically-known paths (`input.charter.nope` errors) — **not** through a dynamically-keyed object (`some _, obs in input.subjects; obs.nope` and `input.subjects[s].nope` are both silent). Subjects are ADR ids, so a fact document is dynamically keyed by construction: the one shape in use is the one shape the checker cannot see. A hand-written `required_facts` set mitigates it only for facts someone remembered to list.

Full measurement record, the reproduction recipe, the falsifiers that would flip the verdict, and the complete 54-line policy: [`docs/proposals/opa-pilot-findings.md`](../proposals/opa-pilot-findings.md).

**Why:** the argument for a policy engine was "once standards compose, a homegrown YAML DSL becomes an accidental programming language" — but #11749 had already normalized the decision into a typed Python protocol, so the alternative to Rego was never a DSL. Adopting it would trade a general-purpose language with a type checker, a debugger and 2300 files of house convention for one with none of that here, and would put the deciding engine outside the import pin that `tests/architecture/test_policy_engine_is_pure.py` holds the reference engine to.


```json:entry
{"id":"OPA-REGO-MEASURED-NOT-ADOPTED-001","source_type":"manual","topic":"dependencies","tags":["opa","rego","policy-engine","decision-seam","not-adopted","issue-11750","epic-11752","adr-0143"],"rule":"OPA/Rego is not a HydraFlow dependency. Pilot #11750 measured it to parity and ruled not-adopted: the Rego was smaller than the Python it replaced (0.71x) and the composition probe was cheaper (0.52x), but the adapter needed to run it was 237 code lines against 84 replaced, break-even ~22 cross-standard rules against 8 declared standards. Do not reintroduce opa, opa eval, .rego policies or an OPA binary without re-running the measurements in docs/proposals/opa-pilot-findings.md.","anti_pattern":"Adding a .rego policy or an `opa eval` shell-out on the argument that policy-as-code is cleaner, without counting the adapter, the per-platform binary, and Rego's fail-open default (a rule referencing an uncollected input key passes `opa check --strict` and silently does not fire; `opa check --schema` catches it only on static paths, never through a dynamically-keyed subjects map)","code_refs":["docs/proposals/opa-pilot-findings.md","src/policy/python_engine.py","src/policy/models.py","tests/architecture/test_policy_engine_is_pure.py","tests/architecture/test_policy_adr_enforcement_parity.py"],"fixed_in_pr":null,"added":"2026-08-29"}
```
