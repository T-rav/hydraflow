...
- **Accepted ADRs:** 91
- **REAL** (real asserting enforcement): 88 (96.7%)
- **WEAK** (prose-only or tautological): 3
- **MISSING** (no `**Enforced by:**`): 0
- **Justified exemptions** (process-only, allow-listed): 3
- **Outstanding unenforced-decision debt** (non-exempt WEAK + MISSING): **0 / 91 = 0.0%**

By declared `**Enforcement:**` kind: `enforced` 88 · `manual` 3 · `decision-of-record` 0.
