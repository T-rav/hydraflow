...
- **Accepted ADRs:** 92
- **REAL** (real asserting enforcement): 89 (96.7%)
- **WEAK** (prose-only or tautological): 3
- **MISSING** (no `**Enforced by:**`): 0
- **Justified exemptions** (process-only, allow-listed): 3
- **Outstanding unenforced-decision debt** (non-exempt WEAK + MISSING): **0 / 92 = 0.0%**

By declared `**Enforcement:**` kind: `enforced` 89 · `manual` 3 · `decision-of-record` 0.
...
| ADR-0118 | REAL | — | enforced | `pytest:tests/architecture/test_import_boundary_gate.py` |
