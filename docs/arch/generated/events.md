...
HydraFlow's `EventBus` fans out **every** published event to **all** subscribers: `subscribe()` / `subscription()` take no event type (`src/events.py`), so subscribers are global — one queue drains the whole bus — not per-`EventType`. The Subscribers column shows `★ all (fan-out)` for every live event; the fan-out consumers are listed below. An event is flagged ⚠️ (likely dead) only when it is a declared `EventType` with **no publisher** (the live-only `EPHEMERAL_EVENT_TYPES` allowlist is exempt).
...
| **SYSTEM_REROUTE** | `src.review_phase._phase:ReviewPhase._review_single_adr`<br>`src.review_phase._phase:ReviewPhase._run_post_verify_advisor_for_adr`<br>`src.triage_phase:TriagePhase._triage_single_traced` | ★ all (fan-out) |
