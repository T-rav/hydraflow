...
| **EPIC_READY** | `src.epic._detail:EpicDetailMixin.refresh_cache`<br>`src.epic._merge_order:EpicMergeOrderMixin._publish_ready_event` | ★ all (fan-out) |
...
| **HITL_UPDATE** | `src.dashboard_routes._hitl_routes:register._resolve_hitl_item`<br>`src.dashboard_routes._hitl_routes:register.hitl_correct`<br>`src.hitl_phase:HITLPhase._process_one_hitl`<br>`src.hitl_runner:HITLRunner.run`<br>`src.pr_unsticker._unsticker:PRUnsticker.unstick` | ★ all (fan-out) |
