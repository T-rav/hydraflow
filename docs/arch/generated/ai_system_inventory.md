...
| `staging_promotion` | `StagingPromotionLoop` | Caretaking | — | — | HITL escalation; PR review + merge gate | Cuts release-candidate snapshots from staging and auto-promotes them to main on green CI. See ADR-0042. |
...
| `term_proposer` | `TermProposerLoop` | Caretaking | — | — | — | Caretaker that grows the ubiquitous-language glossary by detecting load-bearing classes without terms (S1+S2+S5 signals), drafting them via LLM, and opening auto-merging bot PRs as `confidence: proposed`. See ADR-0054. |
