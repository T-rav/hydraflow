...
- **Data plane** — a deliberately boring Anthropic-compatible reverse proxy. Wildcard passthrough of everything under the base path (the CLI hits more than `/v1/messages`; an endpoint whitelist is a hand-maintained list). SSE streams forwarded chunk-for-chunk; upstream errors, 429s, and `overloaded` responses passed through verbatim so client retry behavior is indistinguishable from direct. The less clever the pipe, the more transparent the tap.
...
- **OAuth/subscription session transit** — a true spike, parked with its own question: can (and may) a Max/Team-authenticated Claude Code session ride the proxy? Answered before the org-humans tap is promised. Until then "org-wide" means the org's API-key traffic.
