...
- **A behaviour-neutral `fix(...)` commit is not an escape — paths-scoped, not trailer-only.** The `bug-issue` branch of `escape.detect._classify` is gated on the repo's existing `false_close.has_skip_regression` opt-out trailer (the same P10.6/P10.7 signature, reused rather than reimplemented) AND `false_close.product_paths(commit.changed_paths)` being empty. The trailer alone is NOT sufficient: `Skip-Regression: <why>` also legitimately covers a flaky-test fix, a config-only fix, or a fix already covered by an existing test — real escapes that a trailer-only gate would silently under-count, the one failure mode this falsification instrument must never have. `CommitInfo.changed_paths` (every path the commit touched, any status) feeds the check; when it's unknown (empty — a synthetic `CommitInfo` or an adapter that hasn't populated it) the gate fails OPEN and the candidate is still recorded. A commit that declares `Skip-Regression: <why>` AND carries no product-source delta — e.g. a docs-only diagram refresh — never produces a candidate, even if its subject carries a `fix(...)` prefix. The gate is scoped to the `bug-issue` branch only: a revert or hotfix that happens to also carry the trailer is still recorded. Each suppression logs at `hydraflow.escape_ledger` (info level) so the false-negative surface is observable, not a bare silent `return None`.
