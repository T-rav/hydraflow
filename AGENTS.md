...
### Factory worktree Beads contract

Factory task decomposition does not use a Beads database. In every
implementation worktree, `src/beads_manager.py` reads and writes
`.beads/issues.jsonl` directly:

- JSONL is the sole source of truth for that worktree's phase tasks.
- Every mutation uses a bounded descriptor-relative file lock and atomic
  same-directory replace. Directory, lock, and store descriptors use
  no-follow opens plus inode identity checks, so symlinked or swapped paths
  fail closed without touching their external targets.
- `.beads/` is mode `0700`; `issues.jsonl` and its lock are mode `0600`.
- Existing valid JSONL is preserved byte-for-byte during initialization.
- No server locator, database configuration, or database hook is shipped into
  worktrees. Factory code never probes, starts, migrates, or falls back to an
  external task store.
- Implementer prompts may display a bead ID for phase correlation, but agents
  must not run the database-backed `bd` CLI inside a factory worktree.
- HydraFlow claims root phase tasks before the opaque implementation session.
  After a verified success it claims and closes the ready frontier in
  dependency order, then commits the finalized JSONL before pushing.
  Implementation failures close nothing, leaving unfinished state visible;
  retries validate and reuse any valid existing graph rather than appending
  duplicate phase records.

This is the canonical Beads storage rule for HydraFlow. There is no supported
host/database workflow: agents must not invoke `bd`, migrate or sync Dolt, or
edit `issues.jsonl` themselves.
...
## Beads task state: JSONL only

This section supersedes the repository's former database-backed Beads
instructions. HydraFlow stores factory phase state only in each implementation
worktree's `.beads/issues.jsonl` through `BeadsManager`.

- Never invoke the `bd` CLI from an agent, hook, test, or factory runtime.
- Never start, probe, migrate, pull, or push a Dolt database for task state.
- Never hand-edit `.beads/issues.jsonl`; HydraFlow owns its locked, atomic
  lifecycle transitions.
- Use the issue/PR already supplied by the active workflow for human-visible
  tracking. Do not create a competing task list or database.
...
3. **Verify factory task state** - Let HydraFlow finalize the worktree JSONL
